...
#define MP_SIZE_T_SWAP(x, y)						\
  do {									\
    mp_size_t __mp_size_t_swap__tmp = (mp_size_t)(x);				\
    (x) = (y);								\
    (y) = (int)__mp_size_t_swap__tmp;					\
  } while (0)
...
{
  if (b->_mp_size < 0)
    r->_mp_size = (int)mpz_abs_add_ui (r, b, a);
  else
    r->_mp_size = (int)-mpz_abs_sub_ui (r, b, a);
}
...
mpz_mul_2exp (mpz_t r, const mpz_t u, mp_bitcnt_t bits)
{
  ...
  r->_mp_size = (int)((u->_mp_size < 0) ? - rn : rn);
}
	    ...
	    const mpz_t n, const mpz_t d, enum mpz_div_round_mode mode)
{
  ...
  if (nn < dn)
      ...
      if (qp)
	{
	  ...
	  tq->_mp_size = (int)(qs < 0 ? -qn : qn);
	}
      rn = mpn_normalized_size (np, dn);
      tr->_mp_size = (int)(ns < 0 ? - rn : rn);
...
mpz_powm (mpz_t r, const mpz_t b, const mpz_t e, const mpz_t m)
{
  ...
  if (e->_mp_size < 0)
      ...
      if (b->_mp_size < 0)
	{
      ...
      base->_mp_size = (int)mpn_normalized_size (base->_mp_d, bn);
    }
  mpz_init_set_ui (tr, 1);
	  ...
	  if (tr->_mp_size > mn)
	    {
	      ...
	      tr->_mp_size = (int)mpn_normalized_size (tr->_mp_d, mn);
	    }
	  bit >>= 1;
	}
      while (bit > 0);
...
mpz_probab_prime_p (const mpz_t n, int reps)
{
  ...
  nm1->_mp_size = (int)mpz_abs_sub_ui (nm1, n, 1);
  k = mpz_scan1 (nm1, 0);
...
mpz_abs_add_bit (mpz_t d, mp_bitcnt_t bit_index)
{
  ...
  d->_mp_size = (int)((d->_mp_size < 0) ? - dn : dn);
}
